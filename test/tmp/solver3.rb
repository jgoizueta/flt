...
class FPSolverBase
  ...
  def root(var, parameters={})
    ...
    @guess = (Array(@parameters[var]) + @default_guesses).map{|g| num(g)} # Array(@parameters[var] || @default_guesses).map{|g| num(g)}
    @l_x = @x = @guess.first
    @l_fx = @fx = eval_f(@x)
    @ok = true
    @conv = false
    ...
    if @iteration >= @max_it # TODO: set reason
      @ok = false
  ...
  def eval_f(x)
    @context.math @parameters.merge(@var=>x), &@f
  end
...
if true
solver = SecantSolver.new(Float.context, [0.0, 100.0], Tolerance(3, :decimals)) do |parameters|
  2*parameters[:x]+11.0
end

v = solver.root(:x) # with guess: v = solver.root(:x, :x=>5.0)
puts v

# TODO: if guesses are 0,100 => result is not valid because value at 100 is too large =>
solver = SecantSolver.new(Float.context, [0.0, 10.0], Tolerance(3, :decimals)) do |parameters|
  x,y = parameters.values_at(:x,:y)
  y*exp(x)-10
end

v = solver.root(:x, :y=>2) # with guess: v = solver.root(:x, :x=>5.0, :y=>2)
puts v
...
if true
solver = RFSecantSolver.new(Float.context, [0.0, 100.0], Tolerance(3, :decimals)) do |parameters|
  2*parameters[:x]+11.0
end
...
solver = RFSecantSolver.new(Float.context, [0.0, 10.0], Tolerance(3, :decimals)) do |parameters|
  x,y = parameters.values_at(:x,:y)
  y*exp(x)-10
end
...
class TVM
  ...
  def initialize(tol, context=Float.context)
    ...
    vars = @vars
    tvm = self
    @solver = RFSecantSolver.new(context, [context.Num(0), context.Num(1)], tol) do |parameters|
      tvm.equation(*vars.map{|var| Num(parameters[var])})
    end
    @one = @context.Num(1)
  end
...
if false
solver = RFSecantSolver.new(Float.context, 0, 0, Tolerance(3, :decimals)) do |parameters|
  2*parameters[:x]+11.0
end
...
TESTS2 = [
  [ lambda{|x| atan(x)},0,[[-1,1],[-1,-2],[1,2],[0,0],[0.583312,1],[0.691958,0.791656]]],
]
...
class SolverTests
  ...
  def test(*guesses)
    ...
    context = @context
    f = @f
    solver = @solver_class.new(@context, [x1, x2], @tol) do |parameters|
      context.instance_exec(parameters[:x], &f)
    end
    v = solver.root(:x)
...
=begin
Solver.new(...) {|parameters| sin(parameter[:x])*parameter[:y]}
Solver.new(...) {|parameters|
  x,y = parameters.values_at(:x,:y)
  sin(x)*y
}
